...
function recupereTrame() {
    $ch = curl_init();
    curl_setopt(
    $ch,
    CURLOPT_URL,
    "http://projets-tomcat.isep.fr:8080/appService?ACTION=GETLOG&TEAM=004B");
    ...
    $month = subst($trame,22,2);
    $day = substr($trame,24,2);
    $hour = substr($trame,26,2);
    $min = subst($trame,28,2);
    $sec = subst($trame,30,2);
